...
import { Sprout, ArrowBigUp, CircleCheckBig, CurlyBraces } from "lucide-react";
...
const GardenSession = () => {
    ...
    const fetchQuest = async () => {
        try {
            const res = await AxiosInstance.get(`activity/quest/${getLocalDate(0)}`)
            setQuest(res.data)
    ...
    useEffect(() => {
        const fetchActivity = async () => {
            try {
                const res = await AxiosInstance.get(`/activity/stats/${getLocalDate(0)}`)
            ...
            if (goal > 0) {
                timerRef.current = setInterval(() => {
                    setPrevPlant(prev => {
                        ...
                        let inc = 1
                        if (prev.previous_level !== curPlant.level) inc = 5
                        ...
                        if (goal - prev.previous_exp > inc) {
                            ...
                            const is_next_level = (prev.previous_level < curPlant.level)
                            const next_level = prev.previous_level + 1
                            setForward(false)
                            return {
                                previous_exp: (!is_next_level) ? curPlant.exp : 0,
                                previous_level: (is_next_level) ? next_level : curPlant.level,
                                previous_max_exp: (is_next_level) ? Math.round(100 + (next_level + 1) * (next_level + 1) * (3/8)) : curPlant.max_exp,
                            }
                        }
                        return {
                            previous_exp: inc,
    ...
    return (
        <main className="flex-1 hero-bg p-5 overflow-auto select-none pt-25">
                    ...
                    {curPlant && prevPlant ?
                        <div className="w-full flex flex-col flex-1 gap-5">
                            ...
                            <div className="w-full flex flex-col gap-1">
                                <div className="font-[350] text-[25px] text-black text-left">{prevPlant.previous_level === 20 ? "MAX" : prevPlant.previous_exp  + " / " + prevPlant.previous_max_exp + " EXP"}</div>
                                <div className="w-full h-[30px] rounded-full shadow-md p-2 bg-white">
                                    <div className="bg-gray-200 w-full h-full rounded-full">
                                        <div className={`h-full bg-blue-500 rounded-full ${isForward ? "transition-[width] duration-15 linear" : ""}`}
                                            style={{ width: `${Math.min(prevPlant.previous_exp, prevPlant.previous_max_exp) / prevPlant.previous_max_exp * 100}%` }}></div>
                                    </div>
                                </div>
                            </div>

                            {curPlant.level === 20 ? 
                            <div className="h-[50px] w-fit p-5 px-10 bg-blue-500 text-white rounded-full shadow-md flex items-center justify-center hover:scale-115 transtion-all duration-120"
                            onClick={async () => {
                                try {
                                    await AxiosInstance.post("/activity/garden/harvest")
                                    setPlant(null)
                                    setPrevPlant(null)
                                }
                                catch (e: any) {
                                ...
                                <>
                                    {plantList.map((p, _) =>
                                        <div className="p-5 h-[75px] text-black text-[22px] font-medium flex items-center bg-white/50 rounded-full hover:bg-white/80 hover:scale-110 transition-all duration-100"
                                            onClick={async () => {
                                                try {
                                                    await AxiosInstance.get(`/activity/garden/create/${getLocalDate(0)}/${p}`)
                                                    setQuestFlag(true)
                                                }
                                                catch (e: any) {
                                                    console.log(e.response)
