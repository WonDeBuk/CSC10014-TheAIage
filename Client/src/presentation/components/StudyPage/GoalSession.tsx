      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          ...
          className={`flex-1 h-full ${
            i < difficulty ? colors[i] : "bg-gray-200"
          }`}
        ></div>
      ))}
    </div>
...
const  GoalSession = () => {
    ...
    const [todayGoal, setTodayGoal] = useState<task[]>([
        {title: "--1 naosnfosno knaosdn ansn sodan fokdnaok", desc: "hello\n\n\n\nyour mother \n\n\n\n\n\n\nballs\n\n\n\n\nhahaha\n\n\n\ndod is peak actually", difficulty: 1, is_completed: false},
        {title: "--2", desc: "happy new year", difficulty: 1, is_completed: true},
        {title: "--3", desc: "merry christmas", difficulty: 1, is_completed: false},
        {title: "--4", desc: "happy halloween", difficulty: 1, is_completed: false},
        {title: "--5", desc: "happy thanksgiving", difficulty: 1, is_completed: true},
        {title: "--6", desc: "happy birthday", difficulty: 1, is_completed: false}
    ])
    const [nextGoal, setNextGoal] = useState<task[]>([
        {title: "--1", desc: "hello\n\n\n\nyour mother \n\n\n\n\n\n\nballs\n\n\n\n\nhahaha", difficulty: 1, is_completed: false},
        {title: "--2", desc: "happy new year", difficulty: 1, is_completed: false},
        {title: "--3", desc: "merry christmas", difficulty: 1, is_completed: false},
        {title: "--4", desc: "happy halloween", difficulty: 1, is_completed: false},
        {title: "--5", desc: "happy thanksgiving", difficulty: 1, is_completed: false},
        {title: "--6", desc: "happy birthday", difficulty: 1, is_completed: false}
    ])
    const [miniBar, setMiniBar] = useState<"" | "edit" | "preview">("")
    const [inView, setView] = useState<task | null>(null)

    const [futureTask, setFutureTask] = useState<task>({title: "", desc: "", difficulty: 1, is_completed: false})
    ...
    return (
        <main className="flex-1 hero-bg p-5 overflow-auto select-none">
            <div className="w-full h-full flex items-center">
                <div className="flex-1 h-full flex flex-col justify-center items-center gap-3 pt-20">
                    <div className="text-[30px] font-bold">MỤC TIÊU HÔM NAY</div>
                                ...
                                {todayGoal.filter(g => g.is_completed === false).map((goal, _) =>
                                    <div className="w-full h-[120px] text-black text-[12px] rounded-md bg-white/70 hover:bg-white/90 flex group border border-red-500 hover:scale-95 transition-all duration-150">
                                        ...
                                        <div className="w-[120px] h-[120px] flex justify-center items-center">
                                            <Star size={40} strokeWidth={1.5} className="text-gray-200 hover:scale-150 hover:text-yellow-500 transition-all duration-75"
                                            onClick={() => {
                                            setTodayGoal((prev) => {
                                                const change = [...prev]
                                                const idx = change.findIndex(g => g === goal)
                                                change[idx] = {
                                                    ...change[idx],
                                                    is_completed: true
                                                }
                        ...
                        <div className="w-1/2 h-full flex flex-col items-center p-2 gap-2">
                            ...
                            <div className="w-full h-[660px] flex flex-col gap-3 items-center overflow-y-scroll overflow-x-hidden">
                                {todayGoal.filter(g => g.is_completed).map((goal, _) =>
                                    <div className="w-full h-[120px] text-black text-[12px] rounded-md bg-white/70 hover:opacity-90 hover:scale-95 flex group relative border border-lime-500 transition-all duration-150"
                                    onClick={() => {
                                        const index = todayGoal.findIndex(f => f === goal)
                                        if (index === -1) return
                                        setView(todayGoal[index])
                                        ...
                                        <div className="w-[120px] h-[120px] flex justify-center items-center text-[#00ff00] scale-150"><CheckCheck size={40} strokeWidth={2.5}/></div>
                                    </div>
                                )}
                            </div> :
                            <div className="w-full h-full p-7 flex justify-center items-center opacity-20 bg-white">   
                ...
                <div className="w-[600px] h-full flex flex-col justify-end items-center pt-10 p-5 gap-2">
                    ...
                    <div className={`w-full h-[700px] rounded-md relative bg-white/0 ${miniBar === "" || inView ? "" : "border-2 border-white"}`}>
                        ...
                        <>
                            ...
                            <div className={`w-full h-full flex flex-col justify-start items-start gap-5 p-10 ${inView ? "pointer-events-none" : ""}`}>
                                ...
                                <div className="w-full h-[50px] flex justify-center items-center">
                                    <div className="w-[200px] h-full text-center text-[30px] font-medium bg-gray-100 hover:bg-black hover:scale-120 hover:text-white transition-all duration-150 rounded-md"
                                    onClick={() => {
                                        const newTask: task = {
                                        ...
                                        setFutureTask({title: "", desc: "", difficulty: 1, is_completed: false})
                                    }}>
                                        TẠO
                                    </div>
                                </div>
                            ...
                            <div className="w-full h-full flex flex-col items-center p-10 gap-5">
                                ...
                                <div className="w-full h-full overflow-y-scroll overflow-x-hidden">
                                    ...
                                    <div className="w-full flex flex-col justify-start items-center gap-5">
                                        {nextGoal.map((v, i) =>
                                            <div className="w-full h-[75px] flex bg-white/80 rounded-md text-black items-center hover:scale-x-95 hover:bg-gray-500/25 hover:border-2 hover:border-white duration-75 transition-all px-2">
                                                ...
                                                <div className="w-[60px] h-[60px] flex justify-center items-center"><Eraser size={35} strokeWidth={1.2} 
                                                onClick={() => {
                                                    setNextGoal(prev => {
                                                        const copy = [...prev]
                                                        copy.splice(i, 1)
