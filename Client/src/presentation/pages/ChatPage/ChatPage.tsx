...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    newSocket.on("client_receive_message", (data) => {
      ...
      if (targetUser && targetUser.user_id === data.sender_id)
      ...
      if (searchQuery !== "") {
        const filtered: conversation[] = convList.filter(
          (conv) =>
            conv.other_username
              .toLowerCase()
    ...
    newSocket.on("new_conversation", (data) => {
      ...
      if (
        searchQuery === "" ||
        data.other_username.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="hero-bg h-full w-full flex items-center gap-7 p-5 relative overflow-hidden">
      {isPopup ? (
        <>
          <div
            className="bg-black/20 w-full h-full absolute scale-200"
          ...
          <div className="w-[1200px] h-[800px] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute bg-white rounded-2xl flex flex-col p-3">
            <div className="w-full h-[70px] flex flex-col items-center justify-start">
              <div className="w-full flex-1 flex items-start justify-end">
                <X
                  size={60}
                  strokeWidth={1.6}
      ...
      <div className="h-full flex-1 flex flex-col items-center gap-5 select-none">
        <div className="bg-white w-full h-[75px] rounded-2xl px-3 py-4 flex items-center justify-center gap-2">
          ...
          <input
            ...
            placeholder="Truy tìm đoạn chat."
            ref={searchArea}
            value={searchQuery}
            onChange={(e) => {
          ...
          {filteredConvList.length ? (
            <div className="flex flex-col gap-2 items-center overflow-y-scroll overflow-x-hidden h-full">
              {filteredConvList.map((conv, index) => (
                <div
                  className="w-full flex flex-col justify-start items-center gap-2 p-3"
                  onClick={() => {
                    setTargetUser({
                ...
                >
                  <div
                    className={`w-full flex-1 flex flex-col gap-2 hover:bg-gray-100 cursor-pointer rounded-2xl py-3 px-2 hover:-translate-y-1.5 transition-all duration-150 ${
                      targetFind === conv.other_email
                        ? "bg-gray-200"
                        : "bg-white"
                    }`}
                  >
                    <div className="w-full flex justify-start gap-2 items-center">
                      ...
                      <div className="flex-1 flex flex-col items-start justify-start gap-1">
                        <p className="text-left text-[30px] text-black h-[35px]">
                          {conv.other_username}
                        </p>
                        <p className="text-left text-[15px] text-gray-600 h-5">
                          {conv.other_email}
                        </p>
                      </div>
                    </div>
                    <div className="line-clamp-1 w-full max-h-[30px] text-[18px] text-gray-400 px-2">
            ...
            <div className="px-15 w-full h-full text-[22px] text-gray-400 flex flex-col justify-center items-center">
              ...
              <div
                className="bg-blue-400 text-white rounded-2xl text-[18px] font-medium cursor:pointer text-center w-[150px] h-10 flex justify-center items-center select-none"
                onClick={() => navigate("/counsellors")}
              >
                Tìm kiếm
              </div>
      ...
      {targetUser ? (
        <div className="h-full w-[1200px] rounded-2xl flex flex-col gap-0">
          <div className="w-full h-[100px] flex flex-col gap-0 justify-center items-center">
            <div className="w-full flex-1 flex items-center bg-white rounded-t-2xl px-6 gap-3">
              <div className="w-[90px] h-[90px] flex justify-center items-center">
                <Heart size={60} strokeWidth={1.2} className="text-pink-700" />
              </div>
              ...
              {user?.role === "Counsellor" ? (
                <div
                  className="w-[90px] h-[90px] flex justify-center items-center hover:scale-120 transition-transform duration-100"
                  onClick={async () => {
                    try {
                      setIsPopup(true);
                      const res = await AxiosInstance.get(
                        `/chat/summarize/${targetUser.user_id}`
                      );
                      setRecap(res.data);
                    } catch (e: any) {
                ...
                >
                  <Info size={60} strokeWidth={1.2} className="text-blue-700" />
                </div>
              ) : (
                <></>
              )}
          ...
          <div className="w-full h-[120px] bg-white rounded-b-2xl px-5 py-2">
            <textarea
              className="w-full h-full text-[16px] rounded-2xl bg-gray-100 p-3 overflow-hidden"
              disabled={
                targetUser &&
                targetUser.conversation_id === "" &&
                msgList.length === 1
                  ? true
                  : false
              }
                  ...
                  if (!e.shiftKey) {
                    ...
                    setConvList((prev) => {
                      const index = prev.findIndex(
                        (c) => c.other_user_id == targetUser.user_id
                      );
                      if (index === -1) return prev;
                      const copy = [...prev];
                      const conv = copy.splice(index, 1)[0];
