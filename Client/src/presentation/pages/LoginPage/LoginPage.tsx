...
export default function LoginPage() {
  ...
  return (
    <div className="flex flex-col items-center gap-10 w-dvw h-dvh bg-[#edeffd] fixed top-0">
      <div className="mt-8">
        <p className="text-[36px] text-center">User Login</p>
        ...
        <form className="w-full" onSubmit={handleSubmit}>
          <div className="w-full">
            <div className="w-full mb-3">
              ...
              <input onChange={handleChange} value={formData.email} type="email" name="email" className={`w-full text-sm h-[20px] bg-[#efebef] min-h-10 rounded-lg p-3 ${formData.email ? "text-black" : "text-[#8c8097]"}`} placeholder="example@gmail.com" />
            </div>
            <div className="w-full">
              <label className="block text-[18px] font-[400]" htmlFor="password">Password:</label>
              <input onChange={handleChange} value={formData.password} type="password" name="password" className={`w-full text-sm h-[20px] bg-[#efebef] min-h-10 rounded-lg p-3 ${formData.password ? "text-black" : "text-[#8c8097]"}`} placeholder="Please check for capitalizations and miscellaneous symbols." />
            </div>
          </div>
          <div className="w-full mt-8">
            <input type="submit" className="text-white text-sm bg-black h-[50px] text-center w-full rounded-lg" value="LOGIN"></input>
          </div>
        </form>
      </div>
    </div>
